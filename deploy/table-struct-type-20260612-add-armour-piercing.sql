-- Deploy structs-pg:table-struct-type-20260612-add-armour-piercing to pg
--
-- structsd v0.18.0 adds StructType.primaryWeaponArmourPiercing /
-- secondaryWeaponArmourPiercing (bool) as part of the battleship-vs-tank
-- balancing. struct_type already mirrors every other chain weapon flag
-- (blockable, counterable, ...), so add the two matching boolean columns and
-- re-expose them on view.struct (reproduced verbatim from
-- table-struct-type-20260602-add-generating-rate-precision.sql, with the two
-- new columns slotted next to the *_counterable columns). British spelling
-- "armour" matches the chain field name.
--
-- The cache.handle_event_struct_type importer is updated separately in
-- cache-trigger-add-queue-20260612-armour-piercing.

BEGIN;

    ALTER TABLE structs.struct_type
        ADD COLUMN primary_weapon_armour_piercing   boolean,
        ADD COLUMN secondary_weapon_armour_piercing boolean;
