-- Deploy structs-pg:view-work-20260904-add-location to pg
--
-- Make view.work self-describing for MINE/REFINE: the clock lives on the
-- rig's planet ('12-'||location_id / '13-'||location_id), but the row did
-- not name the location. Add location_type, location_id, and planet_id
-- (planet_id set when location_type = 'planet', else NULL).
    ...
    CREATE OR REPLACE VIEW view.work AS
        WITH work AS (
            ...
            SELECT
                ...
                struct_attribute.object_id as target_id,
                'MINE' as category,
                COALESCE((SELECT planet_attribute.val FROM structs.planet_attribute WHERE planet_attribute.id='12-' || struct.location_id),0) as block_start,
            ...
            SELECT
                ...
                struct_attribute.object_id as target_id,
                'REFINE' as category,
                COALESCE((SELECT planet_attribute.val FROM structs.planet_attribute WHERE planet_attribute.id='13-' || struct.location_id),0) as block_start,
