-- Deploy structs-pg:view-work-20260914-filterable-unions to pg
--
-- Same columns as view-work-20260904-add-location. UNION ALL (categories do
-- not overlap), start BUILD/MINE/REFINE from struct so owner filters use
-- struct_owner_idx, LEFT JOIN fleet for RAID player_id so a missing fleet
-- still yields NULL instead of dropping the planet, and join clocks on PK.
