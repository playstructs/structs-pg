-- Deploy structs-pg:view-struct-20260904-add-state-columns to pg
--
-- Expose hull/status flags and protected_struct_id on view.struct so catalog
-- list / batch object reads can project once instead of N+1 into
-- /api/struct/{id}. health and status were already present; add is_built,
-- is_online, is_destroyed (from structs.struct), and protected_struct_id
-- derived from attr 5- (NULL when unset / zero).
