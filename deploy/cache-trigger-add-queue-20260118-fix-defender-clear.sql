-- Deploy structs-pg:cache-trigger-add-queue-20260118-fix-defender-clear to pg
...
BEGIN;
    ...
    BEGIN
        ...
        ELSIF NEW.composite_key = 'structs.structs.EventStructDefenderClear.structDefenderClearDetail' THEN
            body := (NEW.value)::jsonb;

            DELETE FROM structs.struct_defender WHERE defending_struct_id = body->>'defendingStructId';
            DELETE FROM structs.struct_attribute WHERE id = '5-' || (body->>'defendingStructId')::CHARACTER VARYING;
        ...
        ELSIF NEW.composite_key = 'structs.structs.EventStructType.structType' THEN
            ...
            INSERT INTO structs.struct_type(
                ...
                build_limit ,
                build_difficulty,
                build_draw_p, -- table-struct-type-meta-20251120-add-energy-precision updated this to build_draw_p
                max_health,
                passive_draw_p, -- table-struct-type-meta-20251120-add-energy-precision updated this to passive_draw_p
                possible_ambit,
                movable,
                slot_bound,
                primary_weapon,
            ...
            ELSE
                ...
                CASE split_part(body->>'attributeId', '-',1)
                    ...
                    WHEN '1' THEN
                       ...
                       IF ((body->>'value')::INTEGER & 32) > 0 THEN
                           UPDATE structs.struct SET is_destroyed = 't' WHERE id = split_part(body->>'attributeId', '-', 2) || '-' || split_part(body->>'attributeId', '-', 3);
                       END IF;
                    ELSE
                END CASE;
