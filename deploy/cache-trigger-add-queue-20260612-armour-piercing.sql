-- Deploy structs-pg:cache-trigger-add-queue-20260612-armour-piercing to pg
--
-- cache.handle_event_struct_type is the SQL event handler sync-state invokes
-- to upsert structs.struct_type from chain StructType events. Teach it to
-- read the v0.18.0 primaryWeaponArmourPiercing / secondaryWeaponArmourPiercing
-- fields into the new columns added by
-- table-struct-type-20260612-add-armour-piercing.
--
-- Body reproduced verbatim from
-- cache-trigger-add-queue-20260602-generating-rate-precision.sql with the two
-- boolean fields threaded through the jsonb_to_record column list, the SELECT,
-- the INSERT column list / VALUES, the ON CONFLICT SET, and both sides of the
-- IS DISTINCT FROM change-detection tuples. No sync-state change is needed:
-- it passes the full event JSON and jsonb_to_record only reads declared keys.
...
BEGIN;
    ...
    BEGIN
        SELECT
            ...
            x."primaryWeaponArmourPiercing" AS primary_weapon_armour_piercing,
            ...
            x."secondaryWeaponArmourPiercing" AS secondary_weapon_armour_piercing,
