-- Deploy structs-pg:cache-trigger-add-queue-20260207-add-destroyed-block to pg
                ...
                CASE split_part(v.attribute_id, '-',1)
                    ...
                    WHEN '1' THEN
                        ...
                        IF ((v.value)::INTEGER & 32) > 0 THEN
                            UPDATE structs.struct SET is_destroyed = 't', destroyed_block=(SELECT current_block.height FROM structs.current_block LIMIT 1) WHERE id = split_part(v.attribute_id, '-', 2) || '-' || split_part(v.attribute_id, '-', 3);
                        END IF;
                    ELSE
                    END CASE;
            END IF;
