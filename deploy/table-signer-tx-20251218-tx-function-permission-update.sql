-- Deploy structs-pg:table-signer-tx-20251218-tx-function-permission-update to pg
...
BEGIN;
    ...
    $BODY$
    BEGIN
        PERFORM signer.CREATE_TRANSACTION(_player_id,128,'structs','struct-ore-mine-complete',jsonb_build_array(_struct_id, _proof, _nonce ),'{}');
    END
    $BODY$
    ...
    $BODY$
    BEGIN
        PERFORM signer.CREATE_TRANSACTION(_player_id,128,'structs','struct-ore-refine-complete',jsonb_build_array(_struct_id, _proof, _nonce ),'{}');
    END
    $BODY$
        LANGUAGE plpgsql VOLATILE SECURITY DEFINER COST 100;
    ...
    $BODY$
    BEGIN
        PERFORM signer.CREATE_TRANSACTION(_player_id,128,'structs','planet-raid-complete',jsonb_build_array(_fleet_id, _proof, _nonce ),'{}');
    END
    $BODY$
        LANGUAGE plpgsql VOLATILE SECURITY DEFINER COST 100;
    ...
    $BODY$
    BEGIN
        PERFORM signer.CREATE_TRANSACTION(_player_id,128,'structs','struct-build-complete',jsonb_build_array(_struct_id, _proof, _nonce ),'{}');
    END
    $BODY$
        LANGUAGE plpgsql VOLATILE SECURITY DEFINER COST 100;
