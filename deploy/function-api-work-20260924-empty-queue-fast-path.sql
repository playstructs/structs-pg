-- Deploy structs-pg:function-api-work-20260924-empty-queue-fast-path to pg
--
-- Return from structs.api_work_refresh() before any temp-table DDL when the
-- dirty queue is empty.
--
-- Measured 2026-09-24 after a from-zero sync-state replay of
-- structstestnet-111: 2.77M calls at 2.48 ms mean, the single most expensive
-- statement of the run (6,872 s). The empty-queue path still ran
-- DROP TABLE IF EXISTS + CREATE TEMP TABLE ... ON COMMIT DROP on every call:
-- 2.0 ms, against 0.024 ms for lock + EXISTS + refresh-state stamp.
--
-- Everything after the fast path is unchanged from
-- function-api-work-20260915-incremental-refresh.
...
BEGIN;
    ...
    BEGIN
        ...
        -- Empty queue: stamp and return without temp-table DDL.
        IF NOT EXISTS (SELECT 1 FROM structs.api_work_dirty) THEN
    ...
    COMMENT ON FUNCTION structs.api_work_refresh(BIGINT, TIMESTAMPTZ) IS
        'Incremental per-block refresh of structs.api_work: drains api_work_dirty, recomputes only the affected targets through view.work_live, records model ''work'' in api_refresh_state. Empty queue returns before any temp-table DDL (~0.03 ms). Falls back to api_work_refresh_full() above 2,000 keys. Returns (inserted, updated, deleted).';
